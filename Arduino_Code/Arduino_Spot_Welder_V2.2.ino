...
int autopulsePin = 3;  // Pin to trigger a pulse with a little delay as soon as both welding tips touch the nickel strip
...
void loop() {
  ...
  else
  {
    ...
    // Auto pulse feature (if both welding tips touch the nickel strip the pulse is activated)
    if (digitalRead(autopulsePin) == HIGH && pulse_time > 0) {
